...
import com.hyphenate.easeui.utils.StrangersCachUtil;
import com.hyphenate.exceptions.HyphenateException;
import com.hyphenate.util.EMLog;
import com.hyphenate.util.NetUtils;
import com.zzkx.mtool.MyApplication;
import com.zzkx.mtool.R;
import com.zzkx.mtool.chat.db.AtSupportMessageDao;
import com.zzkx.mtool.chat.db.DemoDBManager;
import com.zzkx.mtool.chat.db.InviteMessgeDao;
import com.zzkx.mtool.chat.db.UserDao;
import com.zzkx.mtool.chat.domain.AtSupportMessage;
import com.zzkx.mtool.chat.domain.EmojiconExampleGroupData;
...
import com.zzkx.mtool.chat.parse.UserProfileManager;
import com.zzkx.mtool.chat.receiver.CallReceiver;
import com.zzkx.mtool.chat.ui.ChatActivity;
import com.zzkx.mtool.chat.ui.EaseMainActivity;
...
import com.zzkx.mtool.chat.utils.PreferenceManager;
...
public class DemoHelper {
    private AtSupportMessageDao atSupportMessageDao;
...
//        avatarOptions.setAvatarShape(1);
        ...
        easeUI.setSettingsProvider(new EaseSettingsProvider() {
            ...
            public boolean isMsgVibrateAllowed(EMMessage message) {
                Boolean silentMode = ChatNotifySoundSettingUtil.getSilentMode();
            ...
            public boolean isMsgSoundAllowed(EMMessage message) {
                Boolean silentMode = ChatNotifySoundSettingUtil.getSilentMode();
                boolean singleSilent = ChatNotifySoundSettingUtil.getSilentMode(message.conversationId());
                return demoModel.getSettingMsgSound() && !silentMode && !singleSilent;
            }
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                if (isVideoCalling) {
                    ...
                    if (chatType == ChatType.Chat) { // single chat searchMessage
                        ...
                        if (chatType == ChatType.GroupChat) {
                            ...
                            if (bean.groupType == EaseConstant.ORDER_CHAT) {
                                ...
                                intent.putExtra(Const.ID, bean.orderId);
                                intent.putExtra(Const.TYPE, bean.orderType);
                            }
                        } else {
                ...
                if (message.getType() == Type.CMD) {
                    ...
                    switch (body.action()) {
                        case EaseConstant.ACTION_AT:
                            break;
                        case EaseConstant.ACTION_ATTENTION:
                            intent.setClass(appContext, NewFriendsMsgActivity.class);
...
//                        intent.setClass(appContext, StateHelperActivity.class);
//                    }
//                }
                return intent;
            }
            ...
            @Override
            public boolean getNotifyInfoShow() {
                return ChatNotifySoundSettingUtil.getShowMessageInfo();
            }
        });
    }
    ...
    private void initDbDao() {
        ...
        atSupportMessageDao = new AtSupportMessageDao(appContext);
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onAutoAcceptInvitationFromGroup(String groupId, String inviter, String inviteMessage) {
            ...
            String st3 = inviter + " " + appContext.getString(R.string.Invite_you_to_join_a_group_chat);
            EaseUser easeUser = getContactList().get(inviter);
            if (easeUser != null) {
                st3 = easeUser.getNick() + " " + appContext.getString(R.string.Invite_you_to_join_a_group_chat);
            }
            EMMessage msg = EMMessage.createReceiveMessage(Type.TXT);
            msg.setChatType(ChatType.GroupChat);
            msg.setFrom(inviter);
...
//                    st3 = extBean.inviterNick + " " + appContext.getString(R.string.Invite_you_to_join_a_group_chat);
//                    StrangersCachUtil.putUser(appContext, inviter, extBean.inviterNick, extBean.inviterHead);
//                }
//            }
...
//            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
            ...
            if (!localUsers.containsKey(username)) {
                ...
                ContactUtil.getInstance(new ContactUtil.CallBack() {
                    ...
                    public void onSuccess(Map<String, EaseUser> contactList) {
                        broadcastManager.sendBroadcast(new Intent(EaseConstantSub.ACTION_CONTACT_CHANAGED));
        ...
        public void onContactInvited(String username, String reason) {
            ...
            // save invitation as searchMessage
            InviteMessage msg = new InviteMessage();
...
//                break;
    ...
    /**
     * save and notify invitation searchMessage
     *
     * @param msg
     */
    ...
    protected void onUserException(String exception) {
        ...
        Intent intent = new Intent(appContext, EaseMainActivity.class);
...
//                            }
                            break;
                        case EaseConstant.ACTION_AT:
//                            cmdMsgBody.getParams()
                            try {
                                String action_info = message.getStringAttribute("action_info");
                                if (!TextUtils.isEmpty(action_info)) {
                                    AtSupportMessage atSupportMessage = Json_U.fromJson(action_info, AtSupportMessage.class);
                                    atSupportMessage.actionType = 0;
                                    atSupportMessage.msgTime = message.getMsgTime();
                                    atSupportMessage.reason = getAtReson(atSupportMessage.atType);
                                    atSupportMessageDao.saveActionMessage(atSupportMessage);
                                    getNotifier().sendNotification(message, false);
                                }
                            } catch (HyphenateException e) {
                                e.printStackTrace();
                        ...
                        case EaseConstant.ACTION_SUPPORT:
                            try {
                                ...
                                if (!TextUtils.isEmpty(action_info)) {
                                    AtSupportMessage atSupportMessage = Json_U.fromJson(action_info, AtSupportMessage.class);
                                    atSupportMessage.actionType = 1;
                                    atSupportMessage.msgTime = message.getMsgTime();
                                    atSupportMessage.reason = getSupportReson(atSupportMessage.supportType);
                                    atSupportMessageDao.saveActionMessage(atSupportMessage);
                                    getNotifier().sendNotification(message, false);
                                }
                            } catch (HyphenateException e) {
...
//                    StateHelperUtil.getInstance().putCmdMessage(message);
    ...
    private String getSupportReson(int supportType) {
        String reson;
        switch (supportType) {
            case 0:
                reson = "赞了我的动态";
    ...
    private String getAtReson(int atType) {
        String reson;
        switch (atType) {
            case 0:
                reson = "在发表动态中@到我";
...
//                    notifyContactsSyncListener(true);
    ...
    public List<AtSupportMessage> getAtMessages() {
        return atSupportMessageDao.getAtMessagesList();
    }

    public List<AtSupportMessage> getSupportMessages() {
        return atSupportMessageDao.getSupportMessageList();
    }

    public void updateActionMessageRead(int id) {
        ContentValues contentValues = new ContentValues();
        contentValues.put(AtSupportMessageDao.COLUMN_NAME_READ_FLAG, 1);
        atSupportMessageDao.updateActionMessage(id, contentValues);
    }
