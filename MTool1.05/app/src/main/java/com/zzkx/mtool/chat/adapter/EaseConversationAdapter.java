...
import com.zzkx.mtool.chat.widget.EaseConversationList.EaseConversationListHelper;
...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String orderHelperContent = null;
        String stateHelperContent = null;
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            if (!TextUtils.isEmpty(description)) {
                ...
                if (groupChatExtBean.groupType == EaseConstant.ORDER_CHAT) {
                    ...
                    holder.avatar.setImageResource(com.zzkx.mtool.R.drawable.em_group_icon);
                }
            } else {
                orderUnreadCount = extBean.unReadCount;
                int type = extBean.conversationType;
                switch (type) {
                    case EaseConstant.ORDER_HELPER:
                        holder.avatar.setImageResource(R.drawable.ic_order_helper);
                        orderHelperContent = extBean.message;
                        break;
                    case EaseConstant.STATE_HELPER:
                        holder.avatar.setImageResource(R.drawable.ic_state_helper);
                        stateHelperContent = extBean.message;
                        break;
                    case EaseConstant.NEW_FRIEND:
                        holder.avatar.setImageResource(R.drawable.ic_new_firend);
                        orderHelperContent = extBean.message;
                        break;
                    case EaseConstant.SYSTEM_NOTI:
                        holder.avatar.setImageResource(R.drawable.ic_64);
                        break;
                    default:
                        holder.avatar.setImageResource(com.zzkx.mtool.R.drawable.em_group_icon);
                }
        ...
        if (orderUnreadCount > 0) {
            holder.unreadLabel.setVisibility(View.VISIBLE);
            holder.unreadLabel.setText(orderUnreadCount + "");
        }
...
//            }
        ...
        } else if (!TextUtils.isEmpty(orderHelperContent)) {
            holder.message.setText(orderHelperContent);
        } else if (!TextUtils.isEmpty(stateHelperContent)) {
            holder.message.setText(stateHelperContent);
        } else {
    ...
    public void notifyDataSetChanged() {
        ...
        System.out.println("conversationList size adapter : " + conversationList.size());
        if (!notiyfyByFilter) {
