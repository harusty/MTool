...
import com.zzkx.mtool.bean.VoiceCallExtBean;
import com.zzkx.mtool.chat.EaseConstantSub;

@SuppressLint("Registered")
public class CallActivity extends ChatBaseActivity {
    ...
    protected void onCreate(Bundle arg0) {
        ...
        pushProvider = new EMCallManager.EMCallPushProvider() {
            ...
            void updateMessageText(final EMMessage oldMsg, final String to) {
                // update local searchMessage text
                ...
                case MSG_CALL_MAKE_VOICE:
                    try {
                        if (msg.what == MSG_CALL_MAKE_VIDEO) {
                            ...
                            VoiceCallExtBean voiceCallExtBean = new VoiceCallExtBean();
                            voiceCallExtBean.groupId = mGroupId;
                            EMClient.getInstance().callManager().makeVoiceCall(username, Json_U.toJson(voiceCallExtBean));
                        }
...
//                        }
    ...
    protected void saveCallRecord() {
        ...
        if (callType == 0)
            message.setAttribute(EaseConstantSub.MESSAGE_ATTR_IS_VOICE_CALL, true);
        else
            message.setAttribute(EaseConstantSub.MESSAGE_ATTR_IS_VIDEO_CALL, true);

        if (isInComingCall) {
//            EaseUser user = StrangersCachUtil.getUser(this,username);
//            message.setAttribute(EaseConstant.USER_HEAD,user.getAvatar());
//            message.setAttribute(EaseConstant.USER_NICK,user.getNick());
            message.setFrom(username);
        }
        // set searchMessage body
