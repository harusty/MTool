...
public class MaterialCheckBox extends View {
    ...
    private void init(Context context) {
        ...
        paintCheck.setColor(doneShapeColor);
        paintCheck.setStrokeWidth(dp2px(2));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRoundRect(rect, baseWidth, baseWidth, paintBorder);
        if (drawRecting) {
            ...
            if (correctProgress > 0) {
                if (correctProgress < 1 / 3f) {
                    ...
                    canvas.drawLine(points[0], points[1], x, y, paintCheck);
                } else {
                    float x = points[4] + (points[6] - points[4]) * correctProgress;
                    float y = points[5] + (points[7] - points[5]) * correctProgress;
                    canvas.drawLine(points[0], points[1], points[2], points[3], paintCheck);
                    canvas.drawLine(points[4], points[5], x, y, paintCheck);
                }
            }
        }
    }
    ...
    private void hideRect() {
        ...
        va.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                paintBorder.setColor(evaluate(c, borderColor, Color.GRAY));
                invalidate();
                if (p >= 1) {
                    isAnim = false;
                    if (listener != null) {
    ...
    private void showRect() {
        ...
        va.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                paintBorder.setColor(evaluate(p, borderColor, backgroundColor));
                invalidate();
                if (p >= 1) {
                    isAnim = false;
                    drawRecting = false;
