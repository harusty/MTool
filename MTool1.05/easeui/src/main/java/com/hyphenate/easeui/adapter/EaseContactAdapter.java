...
import com.hyphenate.easeui.widget.RectChekBox;
...
public class EaseContactAdapter extends ArrayAdapter<EaseUser> implements SectionIndexer {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(mSelectionMode){
            holder.checkbox.setChecked(user.cusSelected);
        }
        ...
        if ((position == 0 || header != null && !header.equals(getItem(position - 1).getInitialLetter())) && !mNoHeader) {
...
//                avatarView.setShapeType(avatarOptions.getAvatarShape());
    ...
    public Object[] getSections() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (list.size() == 0 || (list.get(section) != null && !list.get(section).equals(letter))) {
                list.add(letter);
                section++;
